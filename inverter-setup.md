...
description: >
  Handles cleanup when Predbat genuinely leaves Freeze Charging. Immune to
  Predbat warning/error transitions.
triggers:
  - entity_id: predbat.status
    trigger: state
conditions:
  - alias: "FreezeExit: Freeze guard still armed"
    condition: state
    entity_id: input_boolean.freeze_charge_guard
    state: "on"
  - alias: "FreezeExit: Predbat in a real non-freeze state"
    condition: template
    value_template: |
      {{ not trigger.to_state.state.startswith('Freeze')
         and not trigger.to_state.state.startswith('Warn:')
...
actions:
  ...
  - alias: "FreezeExit: AC handling"
    choose:
      - alias: "FreezeExit: Charging or Hold → AC ON"
        conditions:
          - condition: template
            value_template: |
